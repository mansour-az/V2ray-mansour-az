...
.toolbar{display:flex;gap:9px;flex-wrap:wrap}.table-wrap{overflow:auto;border:1px solid var(--line);border-radius:13px}table{border-collapse:collapse;width:100%;min-width:720px;background:#0e1014}th,td{padding:12px 14px;text-align:right;border-bottom:1px solid #22262e;font-size:13px}th{color:var(--muted);background:#14171c;position:sticky;top:0}tr:hover td{background:#151820}.badge{display:inline-flex;padding:4px 8px;border-radius:999px;background:#243128;color:#77e7ae;font-size:11px}.badge.off{background:#33252a;color:#ff9baa}.empty{text-align:center;color:var(--muted);padding:34px}.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}.check{display:flex;align-items:center;gap:8px;margin:12px 0;color:var(--muted)}.actions{display:flex;gap:8px;flex-wrap:wrap}.toast{position:fixed;left:20px;bottom:20px;background:#20242c;border:1px solid var(--line);padding:12px 16px;border-radius:12px;z-index:10}.mobile-menu{display:none}
@media(max-width:980px){.grid{grid-template-columns:repeat(2,1fr)}.shell{grid-template-columns:1fr}.sidebar{position:fixed;z-index:20;right:0;width:240px;transform:translateX(105%);transition:.2s}.sidebar.open{transform:none}.content{padding:17px}.mobile-menu{display:inline-block}.form-grid{grid-template-columns:1fr}}
@media(max-width:540px){.grid{grid-template-columns:1fr}.topbar h1{font-size:20px}.card{padding:14px}.content{padding:12px}.chart{height:150px}}
...
<script nonce="${nonce}">
const $=s=>document.querySelector(s), fa=n=>Number(n||0).toLocaleString('fa-IR'), date=v=>v?new Date(v).toLocaleString('fa-IR'):'—';let groups=[],visitors=[];
